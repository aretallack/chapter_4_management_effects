...
dat_2 <- read_parquet("./outputs/tables/full_dataframe.parquet")

study_area <- read_sf("./outputs/vector/bonbon_boundary_buffer.gpkg") %>% 
...
# Correlation between difference and rainfall anom

```{r}
cover_rain <- monthly_fc %>% 
  # filter(cover_type == "pv_12mo") %>%
  mutate(bonbon = ifelse(bonbon == 1, "bonbon", "surrounding")) %>% 
  pivot_wider(id_cols = c("date", "cover_type"), values_from = cover, names_from = bonbon) %>% 
  mutate(difference = surrounding - bonbon) %>% 
  merge(monthly_rain, by = "date", all.x = F) %>% 
  mutate(rainfall_anom = rainfall - mean(rainfall, na.rm = T))

ggplot(cover_rain, aes(x = rainfall_anom, y = difference, colour = cover_type)) +
  geom_point() +
...
pv_cor <- cover_rain %>%
  filter(cover_type == "pv_12mo") %>% 
  {cor(.$difference, .$rainfall_anom)}

npv_cor <- cover_rain %>%
  filter(cover_type == "npv_12mo") %>% 
  {cor(.$difference, .$rainfall)}

bare_cor <- cover_rain %>%
  filter(cover_type == "bare_12mo") %>% 
  {cor(.$difference, .$rainfall)}

cat(" PV correlation = ", pv_cor,"\n",
    "NPV correlation = ", npv_cor,"\n",
    "bare correlation = ", bare_cor)
```
...
ggsave("./figures/figure_4_rolling_cover.png",
       width = 16,
       height = 14,
       units = "cm")
