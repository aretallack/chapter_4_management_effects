...
rain <- rast("./outputs/raster/chirps_precipitation/CHIRPS2.0_global_monthly_EW_data_20000101_20230301.nc") %>% 
...
in_out_rain %>% 
  ggplot(aes(x = date, y = value, colour = zone)) +
  geom_line(linewidth = 0.7) +
  annotate("rect", xmin = as_date("2008-10-01"), xmax = as_date("2009-02-01"), ymin = 5, ymax = Inf,
           alpha = .1, fill = "black") +
  annotate("text", x = as_date("2008-08-01"), y = 60, label = "Bon Bon Destocked", hjust = 1, color = "grey30", size = 8/.pt) +
  labs(x = "Year",
...
ggplot(monthly_rain, aes(x = date)) +
  ...
  annotate("rect", xmin = as_date("2008-10-01"), xmax = as_date("2009-02-01"), ymin = 0, ymax = Inf,
           alpha = .1, fill = "black") +
  annotate("text", x = as_date("2008-08-01"), y = 73, label = "Bon Bon Destocked", hjust = 1, color = "grey30", size = 8 / .pt) +
  scale_y_continuous(
  ...
  guides(linetype = guide_legend(ncol = 2, order = 1),
         colour = guide_legend(ncol = 2, override.aes = list(linewidth = 2), order = 2),
         alpha = guide_legend(override.aes = list(linewidth = 1), label.position = "left")
         )
```
