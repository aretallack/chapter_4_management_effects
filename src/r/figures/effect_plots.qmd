...
plot_effects <- function(effects, cover_type, labelx, labely, legend_position) {
  ...
  cover_name <- cover_type %>%
    ...
    get_t_means <- function(x, return_test) {
      if (missing(return_test)) {
      return_test <- F
      }
      
      tmp <- x %>%
        group_by(veg_groups) %>% 
        t.test(values ~ management, data = .)
      means <- tmp$estimate %>% 
        as.vector()
  ...
  ggplot(df, aes(y = values, x = date, colour = veg_groups, linetype = veg_groups)) +
    ...
    # geom_smooth(data = df %>% filter(year(date) <= 2008), method = lm, se = F) +
    geom_segment(data = means, aes(x = min(df$date), y = mean_ref, xend = as_date("2008-09-01"), yend = mean_ref, colour = NULL)) +
    geom_segment(data = means, aes(x = as_date("2009-02-01"), y = mean_eff, xend = max(df$date), yend = mean_eff, colour = NULL)) +
...
p2 <- ggplot(monthly_rain, aes(x = date, y = rainfall)) +
  ...
  scale_x_date(limits = as_date(c(as_date("2001-01-01"), as_date("2023-01-01"))), 
               ...
               expand = c(0, 0)) +
  scale_y_continuous(breaks = seq(0, ceiling(max(monthly_rain$rainfall)), 10),
                     expand = c(0, 1)) +
  labs(y = "Monthly\nRainfall (mm)",
       x = "Year") +
  theme_classic() +
  theme(
    axis.title = element_text(face = "bold"),s
    axis.text = element_text(size = 10)
  )

pv_eff <- plot_effects(effects, "pv05_eff", labelx = ymd("2015", truncated = 2),labely = 1.1, legend_position = c(0.80, 0.11)) +
...
npv_eff <- plot_effects(effects, "npv05_eff", labelx = ymd("2015", truncated = 2),labely = 1.5, legend_position = c(0.80, 0.11)) +
  # geom_line(data = monthly_rain, aes(x = date, y = rainfall_anom/100, colour = NULL, linetype = NULL)) +
  # scale_y_continuous(
  #   name = "Fractional Cover (%)",
  #   sec.axis = sec_axis(~.*100, name = "Monthly Rainfall (mm)")
...
bare_eff <- plot_effects(effects, "bare95_eff", labelx = ymd("2015", truncated = 2),labely = 1.1, legend_position = c(0.18, 0.11)) +
