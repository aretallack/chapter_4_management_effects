...
plot_effects <- function(effects, cover_type, labelx, labely, legend_position) {
  ...
  ggplot(df, aes(y = values, x = date, colour = veg_groups, linetype = veg_groups)) +
    ...
    geom_segment(data = means, aes(x = min(df$date), y = mean_ref, xend = as_date("2008-04-01"), yend = mean_ref, colour = NULL)) +
    geom_segment(data = means, aes(x = as_date("2008-08-01"), y = mean_eff, xend = max(df$date), yend = mean_eff, colour = NULL)) +
    annotate("rect", xmin = as_date("2008-04-01"), xmax = as_date("2008-08-01"), ymin = min(df$values), ymax = Inf,
             alpha = .1, fill = "black") +
...
pv_eff <- plot_effects(effects, "pv05_eff", labelx = ymd("2014-06", truncated = 1),labely = 1.05, legend_position = c(0.80, 0.11)) +
...
npv_eff <- plot_effects(effects, "npv05_eff", labelx = ymd("2014-06", truncated = 1),labely = 1.75, legend_position = c(0.80, 0.11)) +
...
bare_eff <- plot_effects(effects, "bare95_eff", labelx = ymd("2014-06", truncated = 1),labely = 0.2, legend_position = c(0.18, 0.11)) +
