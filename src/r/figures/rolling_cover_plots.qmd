...
ggplot(cover_rain, aes(x = rainfall, y = difference, colour = cover_type)) +
  geom_point() +
...
pv_cor <- cover_rain %>%
  filter(cover_type == "pv_12mo")
pv_cor <- cor(pv_cor$difference, pv_cor$rainfall)

npv_cor <- cover_rain %>%
  filter(cover_type == "npv_12mo")
npv_cor <- cor(npv_cor$difference, npv_cor$rainfall)

bare_cor <- cover_rain %>%
  filter(cover_type == "npv_12mo")
bare_cor <- cor(bare_cor$difference, bare_cor$rainfall)

cat("PV correlation = ", pv_cor,"\n")
cat("NPV correlation = ", npv_cor,"\n")
cat("bare correlation = ", bare_cor,"\n")
