...
ribbon_data <- monthly_fc %>%
  ...
  summarise(max = max(cover, na.rm = T),
            ...
            diff = max - min,
            max_12 = max(min_12, na.rm = T),
            min_12 = min(min_12, na.rm = T),
            diff_12 = max_12 - min_12)
...
p <- ggplot(monthly_rain, aes(x = date)) +
  ...
  annotate("rect", xmin = as_date("2008-01-01"), xmax = as_date("2008-04-01"), ymin = 4, ymax = 72,
           alpha = .1, fill = "black") +
  annotate("rect", xmin = ymd("2008-04-01"), xmax = ymd("2023-01-01"), ymin = 30/coeff, ymax = 70/coeff,
           alpha = .1, fill = "#8DA0CB") +
  ...
  coord_cartesian(ylim = c(0, 55),
                  xlim = c(min(monthly_rain$date), max(monthly_rain$date))) +
  ...
  guides(linetype = guide_legend(ncol = 4, order = 1, title.position = "top", override.aes = list(linewidth = 1)),
         colour = guide_legend(ncol = 4, label.position = "top", order = 1),
...
# Zoomed plot
```{r}
p2 <- ggplot(monthly_fc %>% filter(date >= as_date("2008-01-01")), aes(x = date)) +
  ...
  geom_ribbon(data = ribbon_data %>% filter(date >= as_date("2008-01-01")), aes(ymin = min, ymax = max, fill = cover_type), alpha = 0.2) +
  annotate("rect", xmin = ymd("2008-04-01"), xmax = max(monthly_fc$date), ymin = 30, ymax = 72,
           alpha = 0.1, fill = "#8DA0CB") +
  annotate("text", x = as_date("2017-08-01"), y = 68,
           label = "bold('Bon Bon Destocked')",
           hjust = 1, color = "#8DA0CB", parse = T,
           size = 10/.pt) +
  annotate("rect", xmin = as_date("2008-01-01"), xmax = as_date("2008-04-01"), ymin = 2, ymax = 72,
           alpha = .1, fill = "black") +
  scale_y_continuous(breaks = seq(0, 70, by = 10),
                     position = "right") + 
  coord_cartesian(ylim = c(33, 70),
                  xlim = c(ymd("2008-01-01"), max(monthly_fc$date))) +
  scale_x_date(limits = as_date(c("2008-01-01", max(monthly_fc$date))),
               breaks = "2 year", 
               date_labels = "%Y", 
               expand = c(0, 0)) +
  scale_colour_manual(breaks = c("0bare_mean", "1bare_mean", "0tvc_mean", "1tvc_mean"),
  ...
  theme(
    axis.text = element_text(size = 10),
    axis.title.x = element_text(size = 10, face = "bold", vjust = unit(-0.1, "cm")),
    axis.title.y.right = element_text(vjust = unit(0.2, "cm")),
    axis.title.y.left = element_text(margin = unit(c(0, 0.2, 0, 0), "cm")),
